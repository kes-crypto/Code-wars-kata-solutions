...
let count = (b64) => {
  ...
  let fill = (y,x) => {
    for(let a=[[y,x]],i=0;a[i] && ([y,x]=a[i]);i++) for(let [q,p] of
    [[y-1,x-1],[y-1,x],[y-1,x+1],[y,x-1],[y,x+1],[y+1,x-1],[y+1,x],[y+1,x+1]])
    if(p>=0 && p<w && vis[q] && !vis[q][p] && bgr(col(q,p))) vis[q][p]=1, a.push([q,p])
  }

  let ans = -1
  for(let y=0;y<h;y++) for(let x=0;x<w;x++) if(!vis[y][x] && bgr(col(y,x))) ans++, vis[y][x]=1, fill(y,x)
  return ans
